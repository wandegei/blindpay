...
import { Shield, AlertTriangle, Activity, Users, Zap, Wallet } from "lucide-react";
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    async function load() {
      const [o, t, w, d, l] = await Promise.all([
  ...
  const totalSystemFunds = wallets.reduce((s, w) => s + (w.balance || 0) + (w.locked_balance || 0), 0);
    ...
    <div className="space-y-6 animate-slide-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">Full system oversight and intervention tools</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-2">
            {frozenOrders.length > 0 && (
              <AlertRow type="info" message={`${frozenOrders.length} frozen order(s)`} />
            )}
            {frozenWallets.length > 0 && (
              <AlertRow type="info" message={`${frozenWallets.length} frozen wallet(s)`} />
            )}
            {stuckOrders.length > 0 && (
              <AlertRow type="warning" message={`${stuckOrders.length} stuck order(s) — no activity > 24h`} />
            )}
            {highRiskOrders.length > 0 && (
              <AlertRow type="danger" message={`${highRiskOrders.length} high-risk order(s) (score > 60)`} />
            )}
            {pendingApprovals.length > 0 && (
              <AlertRow type="warning" message={`${pendingApprovals.length} pending approval(s)`} />
            )}
            {disputes.filter(d => d.status === "open").length > 0 && (
              <AlertRow type="danger" message={`${disputes.filter(d => d.status === "open").length} open dispute(s)`} />
            )}
            {frozenOrders.length === 0 && stuckOrders.length === 0 && highRiskOrders.length === 0 && pendingApprovals.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">No active alerts — system healthy</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5">
          ...
          <div className="space-y-2">
            {logs.filter(l => l.actor_role === "admin").slice(0, 8).map(l => (
              <div key={l.id} className="flex items-center gap-3 text-xs">
                <span className="text-muted-foreground w-16 flex-shrink-0">{timeAgo(l.created_date)}</span>
                <span className="text-foreground flex-1">{l.action?.replace(/_/g, " ")}</span>
                <span className="font-mono text-primary">{l.entity_id?.slice(0, 6)}</span>
              </div>
            ))}
            {logs.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">No recent actions</p>
            )}
          </div>
        </div>
      </div>
          ...
          <div className="space-y-2">
            {pendingApprovals.map(tx => (
              <div key={tx.id} className="flex items-center gap-4 p-3 rounded-lg bg-secondary/30">
                ...
                <span className="text-xs font-mono text-primary">{tx.tx_ref || tx.id?.slice(0, 8)}</span>
                <span className="text-xs text-muted-foreground">
                  {tx.from_provider || "External"} → {tx.to_provider || "External"}
                </span>
                <span className="text-xs font-mono font-semibold ml-auto">{formatCurrency(tx.amount, tx.currency)}</span>
              </div>
            ))}
          </div>
        </div>

          ...
          {types.map(t => (
            <button key={t} onClick={() => setFilterType(t)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                filterType === t ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
              }`}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {statuses.map(s => (
            <button key={s} onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-colors ${
                filterStatus === s ? "bg-accent text-accent-foreground" : "bg-secondary text-secondary-foreground hover:bg-secondary/80"
              }`}>
              ...
              <tbody>
                {filtered.map(tx => (
                  <tr key={tx.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                    <td className="px-5 py-3 font-mono text-xs text-primary">{tx.tx_ref || tx.id?.slice(0, 8)}</td>
                    <td className="px-5 py-3"><StatusBadge status={tx.type} /></td>
                    <td className="px-5 py-3 text-xs">
                      <span className="text-foreground">{tx.from_provider || "External"}</span>
                      <span className="text-muted-foreground mx-1">→</span>
                      <span className="text-foreground">{tx.to_provider || "External"}</span>
                    </td>
                    <td className="px-5 py-3 font-mono">{formatCurrency(tx.amount, tx.currency)}</td>
                    <td className="px-5 py-3"><StatusBadge status={tx.status} /></td>
                    <td className="px-5 py-3 text-xs text-muted-foreground">{timeAgo(tx.created_date)}</td>
